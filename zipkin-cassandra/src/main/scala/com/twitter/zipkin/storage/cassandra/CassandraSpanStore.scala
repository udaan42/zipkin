...
class CassandraSpanStore(
  ...
  def getTracesDuration(traceIds: Seq[Long]): Future[Seq[TraceIdDuration]] = {
    ...
    pool {
      (repository.getTraceDuration(true, traceIdSet)
      .map { case (traceId :java.lang.Long, ts :java.lang.Long) => (traceId, ("start", ts))}
      .toSeq ++
        (repository.getTraceDuration(false, traceIdSet)
        .map { case (traceId :java.lang.Long, ts :java.lang.Long) => (traceId, ("end", ts))}
        .toSeq))
      .groupBy(_._1)
      .mapValues(_.map(_._2))
      .map { case (traceId :java.lang.Long, map :Map[Long,Long]) =>
        TraceIdDuration(traceId, map.get("end") - map.get("start"), map.get("start"))
      }
      .toSeq
    }
  }
